...
try:
    # noise_cancellation may be an optional package that can fail to install
    # (network issues). Import it if available, otherwise continue without it.
...
async def entrypoint(ctx: agents.JobContext):
    ...
    session = AgentSession(
        ...
        tts="livekit/en-US-Neural",  # pick your preferred Aura voice
    )
    ...
    await session.start(
        ...
        room_input_options=RoomInputOptions(
            # Enhanced noise cancellation (omit if self-hosting). If the
            # noise_cancellation plugin isn't installed, pass None and
            # continue without enhanced cancellation.
